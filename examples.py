...
to run:
manim examples.py everything -pqh --renderer=opengl --write_to_movie
"""
...
class Example(Scene):
    def construct(self):
        sq = Square(side_length=4, color=GREEN,
...
class graph(Scene):
    def construct(self):
        ...
        func_graph = ax.plot(lambda x: (np.exp(x/10) * np.cos(x))
                             ** 2 + x, x_range=(0, 6, 1), color=RED)
...
class iter_Leslie(Scene):
    def construct(self):
        ...
        I = 100
        for j in range(I):
            ani = [point.animate.apply_function_to_position(lambda x: ax.coords_to_point((param['th1'] * ax.point_to_coords(x)[0] + param['th2'] * ax.point_to_coords(
                x)[1]) * np.exp(-param['lam']*(ax.point_to_coords(x)[0] + ax.point_to_coords(x)[1])), param['p1']*ax.point_to_coords(x)[0])) for point in points]
...
class ph(Scene):
    def construct(self):
        ...
        circles = [Circle(radius=1, fill_color=RED, fill_opacity=0.5).move_to(
            point) for point in points]
        ani = [GrowFromPoint(circle, point)
...
class kmeans(Scene):
    def construct(self):
        ...
        for _ in range(100):
            ani = [point.apply_function_to_position(lambda x: ax.coords_to_point((param['th1'] * ax.point_to_coords(x)[0] + param['th2'] * ax.point_to_coords(
                x)[1]) * np.exp(-param['lam']*(ax.point_to_coords(x)[0] + ax.point_to_coords(x)[1])), param['p1']*ax.point_to_coords(x)[0])) for point in points]
        ...
        for _ in range(10):
            ...
            centers = [VGroup(*[points[i] for i in np.argwhere(shortest == i)[:, 0]])
                       for i in range(len(centeroids))]
            ani = [centeroid.animate.move_to(center.get_center_of_mass()) for centeroid, center in zip(
                centeroids, centers) if not any(np.isnan(center.get_center_of_mass()))]

            self.play(*ani)

            condition = all([all(centeroid.get_center_of_mass() - center.get_center_of_mass() < 1e-6)
...
class numerade1(Scene):
    def construct(self):
        ...
        name = Tex(
            'Distance = (Ave. Velocity) * (Time)').next_to(num_line, DOWN, buff=1)
        self.play(Create(name))
        self.wait()
        ...
        calc = MathTex(
            '0(m) = 6.25 ({m \over s}) * 0 (s)').next_to(name, DOWN, buff=0.5)
        ...
        calc2 = MathTex(
            '25(m) = 6.25 ({m \over s}) * 4 (s)').next_to(name, DOWN, buff=0.5)
...
class logo(SpaceScene):
    def construct(self):
        ...
        v = Tex('V', color=PURE_RED, font_size=300,
                tex_template=TexFontTemplates.urw_zapf_chancery)  # urw_zapf_chancery
...
class Density(ThreeDScene):
    def construct(self):
        ...
        formula2 = MathTex('\\rho = {m \over V}').next_to(
            formula1, DOWN, buff=1)
        ...
        self.add_fixed_in_frame_mobjects(formula1.to_edge(LEFT), formula2.next_to(
            formula1, DOWN, buff=1, aligned_edge=LEFT), v.next_to(formula2, DOWN, buff=1), v2.next_to(v, RIGHT))
        self.wait(3)
        self.remove(v2)
...
class play(ThreeDScene):
    def construct(self):
        ...
        framebox1 = SurroundingRectangle(
            main_formula, buff=.25, color=PURE_RED)
        ...
        framebox2 = SurroundingRectangle(
            main_formula2, buff=.25, color=PURE_RED)
        ...
        d2 = MathTex(
            "\\rho = {4.57 \over {4 \over 3} \\pi r^3} {g \over cm^3}")
        d2b = MathTex(
            "\\rho = {4.57 \over {4 \over 3} \\pi 2^3} {g \over cm^3}")
...
class ani(Scene):
    def construct(self):
        dots = VGroup(*[Dot(color=Color(hue=j/100, saturation=1, luminance=0.5))
                      for j in range(100)]).arrange_in_grid(10, 10)
...
class projectile_motion(Scene):
    def construct(self):
        ...
        table = VGroup(Line([-7, -2, 0], [-5, -2, 0], color=RED), Line(
            [-5, -2, 0], [-5, -4, 0], color=RED), Line([-6, -2, 0], [-5, -1, 0], color=RED))
        ...
        def motion(bullet, t):
            ...
            T = 6
            theta = PI/4
            bullet.move_to([xi+v*np.cos(theta)*(T*t), yi+v *
                           np.sin(theta)*(T*t)-0.49*(T*t)**2, 0])

        self.play(UpdateFromAlphaFunc(bullet, motion))
        self.wait()


class everything(SpaceScene):
    def construct(self):

        # Make mobjects
        s = Square(color=RED, fill_color=GREEN, fill_opacity=1)
        t = Triangle(color=BLUE)
        c = Circle(color=YELLOW_C)
        star = Star(color=ORANGE)
        rect = Rectangle(height=2, width=1)
        poly = RegularPolygon(8, color=GOLD, fill_opacity=0.5)
        dot = Dot()

        # Group mobjects
        shapes = VGroup(s, c, t, star, rect, poly,
                        dot).arrange_in_grid(1, 7).to_corner(UL)

        # add, remove, create a mobject
        self.add(shapes)
        self.remove(s)
        self.play(Create(s))

        # Rotate / scale mobjects
        for i, shape in enumerate(shapes):
            self.play(Rotate(shape, ((-1)**i)*PI/3))
            self.play(shape.animate.scale(i/3))
        self.play(shapes.animate.shift(DOWN))

        # Transform / move mobjects
        c_position = c.get_center()
        self.play(Transform(c, dot))
        self.play(dot.animate.move_to(c_position))

        # move mobject with custom alpha (animation) function
        def motion(mobj, t):
            v = 1
            T = 5
            theta = PI/4
            mobj.move_to(
                [mobj.get_x() + v*np.cos(theta)*(T*t),
                 mobj.get_y() + v*np.sin(theta)*(T*t) - 0.49*(T*t)**2, 0]
            )
        self.play(UpdateFromAlphaFunc(dot, motion))

        # add brace
        shapes2 = VGroup(t, rect, star, poly, c)
        br1 = Brace(shapes2, direction=DOWN)
        lb1 = BraceLabel(br1, 'Mobjects')

        # use updater func to update position of brace / brace label
        lb1.add_updater(lambda mob: mob.next_to(shapes2, DOWN))

        # move, shift, scale group of mobjects at once
        self.add(lb1)
        self.play(Create(lb1))
        self.play(shapes2.animate.move_to(ORIGIN))
        self.play(shapes2.animate.shift(UP))
        self.play(shapes2.animate.scale(2))
        self.play(shapes2.animate.scale(0.5))
        self.play(shapes2.animate.shift(DOWN))
        self.wait()

        # make ground and dynamic bodies
        self.remove(lb1, dot, s)
        ground = Line(LEFT * 10, RIGHT * 10, color=BLACK).to_edge(DOWN)
        self.add(ground)
        self.make_static_body(ground)
        self.make_rigid_body(shapes2)
        self.wait(8)

        # make / remove different texts
        text = Text("Text")
        tex = Tex("Tex")
        mathtex = MathTex("MathTex")
        texts = VGroup(text, tex, mathtex).arrange(DOWN, buff=1).to_edge(UP)
        self.play(Write(texts))
        self.wait(3)
        
        # # Change angle of camera (only works with --renderer=opengl)
        # self.play(
        ...
        def func(x):
            return (np.exp(x/10) * np.cos(x)) ** 2 + x

        func_graph = ax.plot(func, x_range=(0, 6, 1), color=RED)
        graph = VGroup(ax, ax_labels, func_graph)
